...
| 工具 | 版本 | 说明 |
|------|------|------|
| Node.js | ≥ 22 | 与 PROJECT.md / 根包 engines 保持一致 |
| pnpm | ≥ 9 | 包管理器 |
| TypeScript | ≥ 5.0 | 类型支持 |
...
// 1️⃣ 配置 Provider（任意 OpenAI 兼容端点一行接入）
...
const agent = new Agent({
  ...
  router: createRouter([deepseek], deepseek.id),
  tools: [...fsTools],
...
```typescript
permissions: {
  mode: 'allow',   // 自动放行所有操作（开发环境用）
  // mode: 'ask',  // 危险操作弹窗确认
  // mode: 'deny', // 拒绝所有操作
}
```

> mode 仅决定全局默认行为；sdk 还会依据工具 `permission.level`（`read` / `write` / `dangerous`）走内置默认策略：read 默认放行，write / dangerous 默认进入审批。
...
permissions: {
  ...
  rules: [
    ...
    // Shell 命令分类处理
    { tool: 'shell.exec', mode: 'allow', scope: ['ls *', 'cat *', 'git status'] },
    { tool: 'shell.exec', mode: 'ask',   scope: ['npm *', 'pnpm *', 'git add *'] },

    // 禁止删除命令
    { tool: 'shell.exec', mode: 'deny',  scope: ['rm *'] },
  ],
}
```
...
permissions: {
  ...
  rules: [
    { tool: 'fs.readFile',  mode: 'allow' },
    { tool: 'fs.writeFile', mode: 'ask',  scope: ['/project/**'] },
    { tool: 'shell.exec',   mode: 'deny', scope: ['rm *', 'chmod *'] },
  ],
...
这是 LLM 的 ReAct 幻觉问题。AgentNova 通过资源限制从外部兜底：达到 `permissions.limits.maxSteps` / `maxToolCalls` 后会自动终止（默认 50 步 / 100 次调用）。如果需要更细粒度的重复检测，可以在业务侧在 `onBeforeToolCall` 钩子中识别：

```typescript
const recent: Array<{ tool: string; args: string }> = []
agent.hook('onBeforeToolCall', async (ctx) => {
  const sig = `${ctx.toolCall.tool}:${JSON.stringify(ctx.toolCall.args)}`
  recent.push({ tool: ctx.toolCall.tool, args: sig })
  if (recent.length > 30) recent.shift()
  const sameCount = recent.filter(r => r.args === sig).length
  if (sameCount >= 5) {
    return { action: 'deny', reason: `检测到循环调用 ${ctx.toolCall.tool}，已拦截` }
  }
})
```
